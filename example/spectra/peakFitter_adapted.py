...
# parser.add_argument("-d", "--distance", help="minimum peak distance for scipy.find_peaks() (50)", type=float, default=50)
# parser.add_argument(
#     "-r",
#     "--rel_height",
...
def peakFitter(filename, min_prominence=50, min_width=20, rel_height=0.5, distance=50, fit_range_factor=1.75, min_channel=51, smoothing_window=5, plot=True, kafe2_plots=False, verbose=False, print_results=True):
    
    # read data
    if filename.endswith(".hst"):
        hst = np.loadtxt(filename, dtype=np.uint32)
    elif filename.endswith(".npy"):
        hst = np.load(filename)
    hlen = len(hst)
    ...
    if smoothing_window > 1:
        ...
        hst_s = hst
    # search for peaks
    peaks, peak_props = find_peaks(hst_s, prominence=min_prominence, width=min_width, rel_height=rel_height, wlen=350, distance=distance)
    if verbose:
    ...
    for i, p in enumerate(peaks):
        ...
        mus.append(fit.parameter_values[1])
        sigs.append(fit.parameter_values[2])
